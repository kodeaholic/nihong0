...
const { TrialTest } = require('../models');
...
const createItem = async (itemBody) => {
    const item = await TrialTest.create(itemBody);
    return item;
};
...
const queryItems = async (filter, options) => {
    const items = await TrialTest.paginate(filter, options);
    return items;
};
...
const findTestByQuestion = async (question, excludedId) => {
    ...
    const item = await TrialTest.findOne(searchCondition);
    return item;
}
