...
const ListeningBoards = () => {
  const [boards, setBoards] = useState([])
  const [searchKey, setSearchKey] = useState('')
  const [searching, setSearching] = useState(false)
  const [redirect, setRedirect] = useState({ redirect: false, path: '' })
  const refresh = () => {
    listeningBoardService.getBoards().then((res) => {
      setBoards(res.results)
      if (!res.results || !res.results.length)
        toast.success(`Hiện tại chưa có bài nào được thêm`, {
          position: 'top-right',
          autoClose: 2500,
              ...
              <CButton
                ...
                onClick={() => {
                  if (!searchKey.length) {
                    ...
                    setSearching(true)
                    listeningBoardService.getBoards({ title: searchKey }).then((res) => {
                      setSearching(false)
            ...
            {boards &&
              boards.map((item) => (
                <CCol
                  ...
                  onClick={() => {
                    setRedirect({ redirect: true, path: `/listening-boards/getBoard/${item.id}` })
                  }}
                >
...
export default ListeningBoards
