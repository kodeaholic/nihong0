...
import { readingBoardService } from '../../../services/api/readingBoardService'
const ReadingBoards = () => {
  ...
  const refresh = () => {
    readingBoardService.getBoards().then((res) => {
      setBoards(res.results)
      if (!res.results || !res.results.length)
        toast.success(`Hiện tại chưa có bài học nào được thêm`, {
          position: 'top-right',
          autoClose: 2500,
  ...
  const search = () => {
    if (!searchKey.length && !level) {
      ...
      readingBoardService.getBoards(searchObject).then((res) => {
        setSearching(false)
            ...
            {boards &&
              boards.map((item) => (
                <CCol
                  ...
                  onClick={() => {
                    setRedirect({ redirect: true, path: `/reading-boards/getBoard/${item.id}` })
                  }}
                >
